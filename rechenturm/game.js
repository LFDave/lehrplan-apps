...
export const LEVELS = [
  { xp: 0, key: 'steinleger', name: 'Steinleger' },
  { xp: 25, key: 'maurer', name: 'Maurer' },
  { xp: 90, key: 'baumeister', name: 'Baumeister' },
  { xp: 220, key: 'turmwaechter', name: 'Turmwächter' },
  { xp: 500, key: 'turmmeister', name: 'Turmmeister' },
];
...
// Medaillen: Abschluss (Runden), Einsatz (gelöste Aufgaben) und
// App-spezifisch. MA.1.A.3 hat im 3. Zyklus keinen Grundanspruch,
// darum gibt es dort eine Turmspitzen-Medaille für die oberste Stufe.
export const MEDALS = [
  { key: 'runde-1', icon: 'flag', threshold: (s) => s.rounds >= 1, name: 'Erster Stein', desc: 'Eine Runde abgeschlossen.' },
  ...
  { key: 'runden-55', icon: 'trophy', threshold: (s) => s.rounds >= 55, name: 'Rechenturm-Legende', desc: '55 Runden abgeschlossen.' },
  { key: 'aufgaben-50', icon: 'pencil', threshold: (s) => s.tasks >= 50, name: 'Fleissige Hände', desc: '50 Aufgaben gelöst.' },
  { key: 'aufgaben-200', icon: 'zap', threshold: (s) => s.tasks >= 200, name: 'Rechenblitz', desc: '200 Aufgaben gelöst.' },
  { key: 'aufgaben-800', icon: 'gem', threshold: (s) => s.tasks >= 800, name: 'Rechenschatz', desc: '800 Aufgaben gelöst.' },
  { key: 'ga-z1', icon: 'target', threshold: (s) => cleanRuns(s, 'b') >= 1, name: 'Grundanspruch Zyklus 1', desc: 'Stufe b ohne Fehler geschafft.' },
  { key: 'ga-z2', icon: 'target', threshold: (s) => cleanRuns(s, 'f') >= 1, name: 'Grundanspruch Zyklus 2', desc: 'Stufe f ohne Fehler geschafft.' },
  { key: 'turmspitze', icon: 'target', threshold: (s) => cleanRuns(s, 'j') >= 1, name: 'Turmspitze', desc: 'Stufe j ohne Fehler geschafft.' },
