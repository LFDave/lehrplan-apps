// e2e.test.mjs — Playwright end-to-end tests for Wortbau.
//
// Run:
//   cd wortbau/tests && npm install && node e2e.test.mjs
//
...
// Antwort). Grammatik-Fakten, von Hand nachgeprüft.
const QA = {
  "Was für ein Wort ist «Hund»?": "Nomen",
  "Was für ein Wort ist «Blume»?": "Nomen",
  "Was für ein Wort ist «Tisch»?": "Nomen",
  "Was für ein Wort ist «rennen»?": "Verb",
  "Was für ein Wort ist «lachen»?": "Verb",
  "Was für ein Wort ist «singen»?": "Verb",
  "Was für ein Wort ist «schnell»?": "Adjektiv",
  "Was für ein Wort ist «gross»?": "Adjektiv",
  "Was für ein Wort ist «rot»?": "Adjektiv",
  "Was für ein Wort ist «leise»?": "Adjektiv",
  "Welche Zeitform ist «ich spiele»?": "Präsens",
  "Welche Zeitform ist «ich spielte»?": "Präteritum",
  "Welche Zeitform ist «ich habe gespielt»?": "Perfekt",
  "Welche Zeitform ist «wir lachen»?": "Präsens",
  "Haustür = Haus + ?": "Tür",
  "Fussball = Fuss + ?": "Ball",
  "Schulweg = Schule + ?": "Weg",
  "Wie heisst der Wortstamm von «spielen»?": "spiel",
  "Wie heisst der Wortstamm von «fahren»?": "fahr",
  "Welches Wort gehört zur Wortfamilie von «fahren»?": "Fahrrad",
  "Welches Wort gehört zur Wortfamilie von «spielen»?": "Spielplatz",
  "Wie heisst der Infinitiv von «läuft»?": "laufen",
  "Wie heisst der Infinitiv von «gibt»?": "geben",
  "Wie heisst der Infinitiv von «isst»?": "essen",
  "Personalform oder Infinitiv: «gehst»?": "Personalform",
  "Personalform oder Infinitiv: «schwimmen»?": "Infinitiv",
  "Personalform oder Infinitiv: «liest»?": "Personalform",
  "Was ist das Vormorphem in «verlaufen»?": "ver",
  "Was ist das Stammmorphem in «verkaufen»?": "kauf",
  "Verkäufer = ver + käuf + ?": "er",
  "Was für ein Wort ist «sie»?": "Pronomen",
  "Was für ein Wort ist «wir»?": "Pronomen",
  "Was für ein Wort ist «und»?": "Partikel",
  "Was für ein Wort ist «oder»?": "Partikel",
  "Welche Zeitform ist «ich werde spielen»?": "Futur",
  "Welche Zeitform ist «ich hatte gespielt»?": "Plusquamperfekt",
  "Wie viele Fälle hat das Deutsche?": "4",
  "Wie heisst der Wer-Fall?": "Nominativ",
  "Wie heisst der Wen-Fall?": "Akkusativ",
  "Wie heisst der Wem-Fall?": "Dativ",
  "Was für ein Wort ist «unter»?": "Präposition",
  "Was für ein Wort ist «auf»?": "Präposition",
  "Welcher Fall ist «den Hund» in «Ich sehe den Hund.»?": "Akkusativ",
  "Welcher Fall ist «dem Kind» in «Ich helfe dem Kind.»?": "Dativ",
  "Welcher Fall ist «der Hund» in «Der Hund bellt.»?": "Nominativ",
  "Freiheit = frei + ?": "heit",
  "Entdeckung = ent + deck + ?": "ung",
  "Nominalgruppe oder Präpositionalgruppe: «auf dem Tisch»?": "Präpositionalgruppe",
  "Nominalgruppe oder Präpositionalgruppe: «der grosse Hund»?": "Nominalgruppe",
  "Was für ein Wort ist «weil»?": "Konjunktion",
  "Was für ein Wort ist «dass»?": "Konjunktion",
  "Was für ein Wort ist «mit»?": "Präposition",
  "Was für ein Wort ist «hinter»?": "Präposition",
  "Welche Zeitform ist «sie wird lesen»?": "Futur",
  "Welche Zeitform ist «er las»?": "Präteritum",
  "Welche Zeitform ist «wir haben gegessen»?": "Perfekt",
  "Welche Zeitform ist «du liest»?": "Präsens",
  "Welcher Modus ist «Geh nach Hause!»?": "Imperativ",
  "Welcher Modus ist «Er sagt, er sei krank.»?": "Konjunktiv",
  "Welcher Modus ist «Wir lesen ein Buch.»?": "Indikativ",
  "Aktiv oder Passiv: «Der Kuchen wird gebacken.»?": "Passiv",
  "Aktiv oder Passiv: «Anna backt einen Kuchen.»?": "Aktiv",
  "Was ist das Subjekt in «Der Hund jagt die Katze.»?": "der Hund",
  "Was ist das Objekt in «Der Hund jagt die Katze.»?": "die Katze",
  "Einfach oder zusammengesetzt: «Ich lese, weil es regnet.»?": "zusammengesetzt",
  "Einfach oder zusammengesetzt: «Ich lese.»?": "einfach",
};
...
/* ── Data and copy sanity ─────────────────────────────────────────── */
{
  ...
  check("data: GA marks on a and c and e",
    STUFEN.filter((s) => s.ga).map((s) => s.id).join(",") === "a,c,e");
  const eszett = [];
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(41 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Wortbau");
...
check("round c: GA medal for Zyklus 2", (await page.textContent(".done")).includes("Grundanspruch Zyklus 2"));
await page.click('[data-action="home"]');
await page.waitForSelector(".stufen-list");

await playRound("e");
check("round e: completion shows XP", (await page.textContent(".reward-xp")).includes(`+${roundXp("e", 8)} XP`));
check("round e: GA medal for Zyklus 3", (await page.textContent(".done")).includes("Grundanspruch Zyklus 3"));
await page.click('[data-action="home"]');
await page.waitForSelector(".stufen-list");

/* ── Persistence, mistake flow, reset ─────────────────────────────── */
await page.waitForSelector(".stats-strip");
const expectedXp = roundXp("a", 8) + roundXp("c", 8) + roundXp("e", 8);
