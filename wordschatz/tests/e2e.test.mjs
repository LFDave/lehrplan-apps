// e2e.test.mjs — Playwright end-to-end tests for Wordschatz.
//
// Run:
//   cd wordschatz/tests && npm install && node e2e.test.mjs
//
...
// Unabhängig neu aufgeschriebenes Wörterbuch Englisch ↔ Deutsch.
const EN2DE = {
  "hello": "hallo", "thank you": "danke", "yes": "ja", "no": "nein",
  "goodbye": "auf Wiedersehen", "one": "eins", "two": "zwei", "three": "drei",
  "red": "rot", "blue": "blau", "the cat": "die Katze", "the dog": "der Hund",
  "the mother": "die Mutter", "the father": "der Vater", "the school": "die Schule",
  "the book": "das Buch", "the pencil": "der Bleistift", "the bread": "das Brot",
  "the milk": "die Milch", "the apple": "der Apfel", "Monday": "Montag",
  "Tuesday": "Dienstag", "green": "grün", "black": "schwarz", "ten": "zehn",
  "it is raining": "es regnet", "the sun": "die Sonne", "today": "heute",
  "tomorrow": "morgen", "yesterday": "gestern", "to eat": "essen",
  "to drink": "trinken", "to play": "spielen", "big": "gross", "small": "klein",
  "the week": "die Woche",
  "I like": "ich mag", "I would like": "ich möchte", "the weekend": "das Wochenende",
  "the homework": "die Hausaufgaben", "the family": "die Familie",
  "the holidays": "die Ferien", "to work": "arbeiten", "to live": "wohnen",
  "to buy": "kaufen", "the city": "die Stadt", "the morning": "der Morgen",
  "How are you?": "Wie geht es dir?", "I don't understand.": "Ich verstehe nicht.",
  "What time is it?": "Wie spät ist es?",
  "I live in Berne.": "Ich wohne in Bern.", "I like playing football.": "Ich spiele gern Fussball.",
  "See you tomorrow.": "Wir sehen uns morgen.",
};
const DE2EN = Object.fromEntries(Object.entries(EN2DE).map(([e, d]) => [d, e]));

function solveTyped(expr) {
  const m = expr.match(/^Übersetze auf Englisch: (.+)$/);
  return m ? DE2EN[m[1]] ?? null : null;
}

function chooseOption(expr, options) {
  let m;
  if ((m = expr.match(/^Was heisst «(.+)» auf Deutsch\?$/))) {
    return options.indexOf(EN2DE[m[1]]);
  }
  if ((m = expr.match(/^Wie heisst «(.+)» auf Englisch\?$/))) {
    return options.indexOf(DE2EN[m[1]]);
  }
  return -1;
}
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(47 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Wordschatz");
