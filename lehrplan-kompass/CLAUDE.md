...
- Tests: `cd tests && npm install && node e2e.test.mjs` — must pass
  ...
  external requests.
- Practice-app links: `PRACTICE_APPS` in `data.js` maps official codes to sibling apps; rows with an entry render an "Üben mit ..." link below the toggle (relative href for sibling apps; the Wortwerkstatt entry uses an absolute same-origin URL because it lives in the small-apps repo; never inside the button element).
