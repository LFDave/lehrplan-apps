...
Stromkreis ist ein Übungsmodul der Lehrplan-Familie und folgt dem
...
Umgesetzt ist NT.5.2 (Physik): Grundlagen der Elektrik — Wirkungen
des Stroms (a), Serie- und Parallelschaltung (b), das Ohmsche Gesetz
mit Stromstärke, Spannung und Widerstand (c), Knoten- und
Maschenregel im verzweigten Kreis (d) sowie Elektromotor und
Generator (e).

**Bewusste Übersetzung:** Experimente am Stromkreis sind als Regeln, Rechnungen und
Vorhersagen umgesetzt (Was passiert, wenn eine Lampe ausfällt?).
...
- Lehrplan 21, Ausgabe Kanton Bern, Stand 01.08.2022 (Datum in der
  ...
  Kompetenzaufbau NT.5.2. Die Stufen a bis e sind die offiziellen
...
Das Ohmsche Gesetz (alle drei Varianten, ganzzahlig) und die
Knotenregel werden generiert und von der Suite mit eigener
Physik-Rechnung unabhängig nachgerechnet; die Wissens-Stufen liegen
in festen Pools mit neu aufgeschriebener Antwort-Tabelle.
...
- `localStorage`-Schlüssel `stromkreis.progress`; Reset im Footer mit
...
Cache-Busting `?v=12`. Bewegung nur als
...
Zuordnung: a, e → strom-wirkungen; b, d → schaltungen; c → ohmsches-gesetz.
