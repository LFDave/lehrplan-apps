// e2e.test.mjs — Playwright end-to-end tests for Wordbau.
//
// Run:
//   cd wordbau/tests && npm install && node e2e.test.mjs
//
...
// Unabhängige Nachrechnung: eigene Plural-Regel (unregelmässige
// Formen zuerst, sonst -s), eigene he-Form-Regel, dazu eine neu
// aufgeschriebene Antwort-Tabelle für die festen Formen.
const IRREGULAR_PLURAL = { man: "men", woman: "women", child: "children", foot: "feet", tooth: "teeth", mouse: "mice" };

const QA = {
  "a oder an: ... apple?": "an",
  "a oder an: ... orange?": "an",
  "a oder an: ... egg?": "an",
  "a oder an: ... umbrella?": "an",
  "a oder an: ... dog?": "a",
  "a oder an: ... house?": "a",
  "a oder an: ... banana?": "a",
  "Wie sagst du «Ich heisse Ben» auf Englisch?": "My name is Ben.",
  "Wie sagst du «Guten Morgen»?": "Good morning",
  "«Danke» heisst auf Englisch ...?": "thank you",
  "Wie fragst du «Wie geht es dir?»?": "How are you?",
  "be: I ...": "am",
  "be: you ...": "are",
  "be: he ...": "is",
  "be: we ...": "are",
  ...
  "have: he ...": "has",
  "have: we ...": "have",
  "Ben ist ein Junge: ... is happy.": "He",
  "Mia ist ein Mädchen: ... is happy.": "She",
  "Der Hund: ... is small.": "It",
  "Ben und ich: ... are friends.": "We",
  "Ben und Mia: ... are friends.": "They",
  "Fragewort «Wo»: ... do you live?": "Where",
  "Fragewort «Wann»: ... is your birthday?": "When",
  "Fragewort «Wer»: ... is that?": "Who",
  "Fragewort «Was»: ... is this?": "What",
  "Fragewort «Warum»: ... are you sad?": "Why",
  "Fragewort «Wie alt»: ... are you?": "How old",
  "he-Form von «watch»?": "watches",
  "he-Form von «go»?": "goes",
  "he-Form von «do»?": "does",
  "The book is ... the table. (auf)": "on",
  "The cat is ... the box. (in)": "in",
  "The ball is ... the bed. (unter)": "under",
  "We are ... school. (in der Schule)": "at",
  "... Monday (am Montag)": "on",
  "... July (im Juli)": "in",
  "Welcher Satz ist richtig gebaut?": "I play football every day.",
  "Welche Frage ist richtig gebaut?": "Where do you live?",
  "Ich mag keinen Tee: I ... like tea.": "don't",
  "Sie mag keinen Tee: She ... like tea.": "doesn't",
  "Verneine «I am happy»:": "I am not happy.",
  "Ich kann schwimmen: I ... swim.": "can",
  "Ich muss gehen: I ... go.": "must",
  "Kannst du helfen? ... you help?": "Can",
  "this oder these: ... book?": "this",
  "this oder these: ... books?": "these",
  "that oder those: ... tree there?": "that",
  "that oder those: ... trees there?": "those",
};

function solveTyped(expr) {
  let m;
  if ((m = expr.match(/^Plural: one (\S+), two \.\.\.$/))) {
    return IRREGULAR_PLURAL[m[1]] ?? m[1] + "s";
  }
  if ((m = expr.match(/^Setze die he-Form ein: (\S+) → he \.\.\.$/))) {
    return m[1] + "s";
  }
  return QA[expr] ?? null;
}
...
/* ── Data and copy sanity ─────────────────────────────────────────── */
{
  ...
  check("data: GA marks on a and c",
    STUFEN.filter((s) => s.ga).map((s) => s.id).join(",") === "a,c");
  const eszett = [];
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(66 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Wordbau");
...
check("round a: GA medal for Zyklus 2", (await page.textContent(".done")).includes("Grundanspruch Zyklus 2"));
...
const expectedXp = roundXp("a", 8) + roundXp("c", 8);
