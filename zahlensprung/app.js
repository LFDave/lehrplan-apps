...
function renderTask() {
  ...
  if (task.type === 'count') {
    taskHtml = `
      <h2 class="task-question">${t('task.count')}</h2>
      <div class="dots" role="img" aria-label="${task.dots} Punkte">
        ${Array.from({ length: Math.ceil(task.dots / 5) }, (_, g) => `<span class="dot-group">${'<span class="dot"></span>'.repeat(Math.min(5, task.dots - g * 5))}</span>`).join('')}
      </div>
      ${typedInput(task.answer)}
    `;
  } else if (task.type === 'sequence') {
    taskHtml = `
      <h2 class="task-question">${t('task.sequence')}</h2>
      <p class="sequence">${task.terms.map((x) => `<span class="term">${x}</span>`).join('')}<span class="term blank">?</span></p>
      ${typedInput(task.answer)}
    `;
  } else if (task.type === 'order') {
    taskHtml = `
      <h2 class="task-question">${t('task.order')}</h2>
      ...
      <button class="btn secondary small" data-action="undo" aria-label="${esc(t('task.undo'))}">${icon('delete')}</button>
      <p class="advisory">${t('task.orderhint')}</p>
    `;
  } else {
    taskHtml = `
      <h2 class="task-question">${t('task.estimate')}</h2>
