// game.js — XP, Levels und Medaillen für Zahlensprung. Reine Funktionen
...
export const LEVELS = [
  { xp: 0, key: 'huepfer', name: 'Hüpfer' },
  { xp: 25, key: 'springer', name: 'Springer' },
  { xp: 90, key: 'weitspringer', name: 'Weitspringer' },
  { xp: 220, key: 'ueberflieger', name: 'Überflieger' },
  { xp: 500, key: 'zahlenakrobat', name: 'Zahlenakrobat' },
];
...
// Medaillen: Abschluss (Runden), Einsatz (gelöste Aufgaben, auch mit
// Fehlern) und App-spezifisch (Grundansprüche, alle Stufen entdeckt).
export const MEDALS = [
  { key: 'runde-1', icon: 'flag', threshold: (s) => s.rounds >= 1, name: 'Erste Runde', desc: 'Eine Runde abgeschlossen.' },
  ...
  { key: 'runden-55', icon: 'trophy', threshold: (s) => s.rounds >= 55, name: 'Zahlensprung-Legende', desc: '55 Runden abgeschlossen.' },
  { key: 'aufgaben-50', icon: 'pencil', threshold: (s) => s.tasks >= 50, name: 'Fleissige Hände', desc: '50 Aufgaben gelöst.' },
  { key: 'aufgaben-200', icon: 'zap', threshold: (s) => s.tasks >= 200, name: 'Zahlenblitz', desc: '200 Aufgaben gelöst.' },
  { key: 'aufgaben-800', icon: 'gem', threshold: (s) => s.tasks >= 800, name: 'Zahlenschatz', desc: '800 Aufgaben gelöst.' },
  { key: 'ga-z1', icon: 'target', threshold: (s) => cleanRuns(s, 'c') >= 1, name: 'Grundanspruch Zyklus 1', desc: 'Stufe c ohne Fehler geschafft.' },
  { key: 'ga-z2', icon: 'target', threshold: (s) => cleanRuns(s, 'g') >= 1, name: 'Grundanspruch Zyklus 2', desc: 'Stufe g ohne Fehler geschafft.' },
  { key: 'ga-z3', icon: 'target', threshold: (s) => cleanRuns(s, 'j') >= 1, name: 'Grundanspruch Zyklus 3', desc: 'Stufe j ohne Fehler geschafft.' },
