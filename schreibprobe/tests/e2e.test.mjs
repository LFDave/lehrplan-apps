// e2e.test.mjs — Playwright end-to-end tests for Schreibprobe.
//
// Run:
//   cd schreibprobe/tests && npm install && node e2e.test.mjs
//
...
// Unabhängig neu aufgeschriebene Liste der FALSCHEN Varianten. Bei
// "Welche Schreibung stimmt?"-Fragen ist die richtige Option die, die
// hier NICHT steht; bei der Fehlersuche die, die hier steht.
const WRONG = new Set([
  "der Hund bellt.", "wir spielen draussen.", "heute regnet es.",
  "die katze", "der ball", "anna lacht.",
  "Es regnet", "Der Tag ist schön",
...
// Eindeutige Fragen (Frage → richtige Antwort), unabhängig aufgeschrieben.
const QA = {
  "Schreibe das Tier richtig: hund": "Hund",
  "Schreibe den Namen richtig: anna": "Anna",
  "Schreibe richtig: Schtein": "Stein",
  "Schreibe richtig: Schpiel": "Spiel",
  "Schreibe richtig: schpät": "spät",
  ...
  "Du bist unsicher: «komen» oder «kommen»? Was hilft?": "auf den kurzen Vokal achten",
  "Anna ___ einen Apfel.": "isst",
  "Der Hund ___ im Garten.": "ist",
  "Papa ___ eine Suppe.": "isst",
  "Ich hoffe, ___ du kommst.": "dass",
  "Ich sehe ___ Haus.": "das",
  "Er weiss, ___ es spät ist.": "dass",
  "Wie viele Fehler hat der Satz: «der hund belt.»?": "3",
  "Wie viele Fehler hat der Satz: «wir spielen fussball.»?": "2",
  "Wie viele Fehler hat der Satz: «Die katze schläft.»?": "1",
  "Wie viele Fehler hat der Satz: «Das Velo ist rot.»?": "0",
};
...
function chooseOption(expr, options) {
  ...
  if (expr === "Welches Wort ist falsch geschrieben?") {
    return options.findIndex((o) => WRONG.has(o));
  }
  // Generische Richtig/Falsch-Fragen: richtig ist, was nicht falsch ist.
  const idx = options.findIndex((o) => !WRONG.has(o));
  const wrongCount = options.filter((o) => WRONG.has(o)).length;
  return wrongCount === options.length - 1 ? idx : -1;
}
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(37 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
const expectedXp = roundXp("a", 8) + roundXp("d", 8) + roundXp("f", 8);
