...
- A pool that a Stufe draws from on its own holds clearly more items than a round has tasks (at least 12 for a round of 8), so two rounds never show the same set; `genRound` only prevents repeats within one round. `node tools/pool-audit/audit.mjs` checks every app against this rule.
