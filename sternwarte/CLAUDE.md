...
- **Guiding principle: one app = one Lehrplan 21 competency (NMG.4.5);
...
- Content lives in pools in `gen.js`; every entry needs its
  ...
  the same change. Factual content must stay rock-solid — established
  astronomy basics only, no invented facts, no ambiguous options.
- Each Stufe's pool must yield at least 8 distinct tasks (round
  length; the suite asserts it).
- Storage key `sternwarte.progress`; Kompass links here via
  `PRACTICE_APPS['NMG.4.5']` in `lehrplan-kompass/data.js`.
