...
Motbau ist ein Übungsmodul der Lehrplan-Familie und folgt dem
...
Umgesetzt ist FS1F.5.D.1 (Französisch, Grammatik): erste Bausteine
mit Artikeln, Mehrzahl und Wendungen (a), Konjugation der Verben auf
-er, Personalpronomen und Fragewörter (b), être und avoir,
unregelmässige Mehrzahl, Präpositionen und Wortfolge (c) sowie
Verneinung, Modalverben, Demonstrativ- und Reflexivpronomen (d).
...
- Lehrplan 21, Ausgabe Kanton Bern, Stand 01.08.2022 (Datum in der
  ...
  Kompetenzaufbau FS1F.5.D.1. Die Stufen a bis d sind die offiziellen
...
- Sprachliche Inhalte sind auf gesicherte Grundformen beschränkt
  (regelmässige Mehrzahl, -er-Konjugation, être/avoir, häufige
  Präpositionen).

## Inhalt und Aufgabenformen

Die regelmässige Mehrzahl und die -er-Konjugation werden generiert
und von der Suite mit eigener Endungs-Tabelle und Mehrzahl-Regel
unabhängig nachgerechnet; alle übrigen Formen liegen in festen Pools
mit neu aufgeschriebener Antwort-Tabelle. Getippt werden nur Formen
ohne Sonderzeichen jenseits der Schweizer Tastatur; œ und êtes
erscheinen nur zur Auswahl.
...
- `localStorage`-Schlüssel `motbau.progress`; Reset im Footer mit
...
Lucide-Icons inline, Cache-Busting `?v=8`. Bewegung nur als
...
Zuordnung: a, b, d → franzoesisch-saetze; c → verben-fr.
...
Zufall gegen ein unabhängiges Orakel (eigene Endungs-Tabelle und
Mehrzahl-Regel), dazu die UI-Abläufe (Runden auf den GA-Stufen,
