// e2e.test.mjs — Playwright end-to-end tests for Motbau.
//
// Run:
//   cd motbau/tests && npm install && node e2e.test.mjs
//
...
// Unabhängige Nachrechnung: eigene Endungs-Tabelle für die Verben
// auf -er, eigene Mehrzahl-Regel (-al wird -aux, sonst -s), dazu
// eine neu aufgeschriebene Antwort-Tabelle für die festen Formen.
const ENDUNGEN = { je: "e", tu: "es", il: "e", elle: "e", nous: "ons", vous: "ez", ils: "ent", elles: "ent" };

const QA = {
  "le oder la: ... maison (das Haus)?": "la",
  "le oder la: ... chien (der Hund)?": "le",
  "le oder la: ... fleur (die Blume)?": "la",
  "le oder la: ... livre (das Buch)?": "le",
  "le oder la: ... table (der Tisch)?": "la",
  "le oder la: ... vélo (das Velo)?": "le",
  "un oder une: ... fille (ein Mädchen)?": "une",
  "un oder une: ... garçon (ein Junge)?": "un",
  "Wie sagst du «Ich heisse Anna» auf Französisch?": "Je m'appelle Anna.",
  "Wie sagst du «Guten Tag»?": "Bonjour",
  "«Danke» heisst auf Französisch ...?": "merci",
  "«Auf Wiedersehen» heisst ...?": "au revoir",
  "Marie singt: ... chante.": "elle",
  "Paul tanzt: ... danse.": "il",
  "Paul und Marie spielen: ... jouent.": "ils",
  "Marie und Anna singen: ... chantent.": "elles",
  "Du und ich, wir spielen: ... jouons.": "nous",
  "Fragewort «Wer»: ... est-ce?": "Qui",
  "Fragewort «Wo»: ... habites-tu?": "Où",
  "Fragewort «Wann»: ... viens-tu?": "Quand",
  "Fragewort «Warum»: ... pleures-tu?": "Pourquoi",
  "Fragewort «Wie»: ... ça va?": "Comment",
  "Fragewort «Was»: ... fais-tu?": "Que",
  "être: je ...": "suis",
  "être: tu ...": "es",
  "être: il ...": "est",
  "être: nous ...": "sommes",
  ...
  "avoir: vous ...": "avez",
  "avoir: ils ...": "ont",
  "Mehrzahl von «l'œil» (das Auge)?": "les yeux",
  "Je vais ... école. (in die Schule)": "à l'",
  "Je vais ... cinéma. (ins Kino)": "au",
  "Je vais ... piscine. (ins Schwimmbad)": "à la",
  "Il habite ... Suisse. (in der Schweiz)": "en",
  "Welcher Satz ist richtig gebaut?": "Je mange une pomme.",
  "Welche Frage ist richtig gebaut?": "Où habites-tu?",
  "Verneine «Je chante»:": "Je ne chante pas.",
  "Verneine «Il joue»:": "Il ne joue pas.",
  "Je ... sais pas. Welches Wort fehlt?": "ne",
  "pouvoir (können): je ...": "peux",
  "pouvoir: il ...": "peut",
  "vouloir (wollen): je ...": "veux",
  "vouloir: il ...": "veut",
  "Ich will spielen: Je ... jouer.": "veux",
  "ce, cette oder ces: ... livre (dieses Buch)?": "ce",
  "ce, cette oder ces: ... maison (dieses Haus)?": "cette",
  "ce, cette oder ces: ... enfants (diese Kinder)?": "ces",
  "Je ... lave. (ich wasche mich)": "me",
  "Tu ... laves. (du wäschst dich)": "te",
  "Il ... lave. (er wäscht sich)": "se",
};

function solveTyped(expr) {
  let m;
  if ((m = expr.match(/^Mehrzahl: (?:le|la|l')\s?(\S+) → les \.\.\.$/))) {
    const wort = m[1];
    return wort.endsWith("al") ? wort.slice(0, -2) + "aux" : wort + "s";
  }
  if ((m = expr.match(/^Konjugiere (\S+)er: (je|tu|il|elle|nous|vous|ils|elles) \.\.\.$/))) {
    return m[1] + ENDUNGEN[m[2]];
  }
  return QA[expr] ?? null;
}
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(65 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Motbau");
check("home: all Stufen with GA badges",
  await page.locator(".stufe").count() === 4 && await page.locator(".ga-badge").count() === 2);
check("home: competency code visible", (await page.textContent('[data-stufe="b"]')).includes("FS1F.5.D.1.b"));
check("home: Merkblatt link Stufe c",
  await page.locator('.merkblatt-link[href="../merkheft/verben-fr.html"]').count() === 1);
...
await page.goto(`${URL}?stufe=c`);
await page.waitForSelector(".task-area");
check("deep link: ?stufe=c starts the Stufe directly, query cleaned",
