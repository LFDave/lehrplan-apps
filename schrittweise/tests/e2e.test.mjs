// e2e.test.mjs — Playwright end-to-end tests for Schrittweise.
//
// Run:
//   cd schrittweise/tests && npm install && node e2e.test.mjs
//
...
// Unabhängige Nachrechnung: eigener kleiner Interpreter für die
// generierten Anleitungen und Programme, dazu eine neu
// aufgeschriebene Antwort-Tabelle für die Wissensfragen.
const QA = {
  "Backrezept: «Rühre zuerst den Teig, backe ihn danach.» Was kommt zuerst?": "den Teig rühren",
  "Die Bastelanleitung hat die Schritte 1 bis 4. Wo beginnst du?": "bei Schritt 1",
  "Tanzschritt: «Zwei Schritte nach links, dann klatschen.» Was kommt nach den Schritten?": "klatschen",
  "Wozu ist eine Spielanleitung da?": "damit alle nach den gleichen Regeln spielen",
  "Du lässt beim Rezept einen Schritt aus. Was kann passieren?": "das Ergebnis stimmt nicht",
  "Du probierst einen Weg durchs Labyrinth und landest in einer Sackgasse. Was machst du?": "zurückgehen und einen anderen Weg probieren",
  "Wie prüfst du, ob dein Lösungsweg stimmt?": "ihn Schritt für Schritt ausprobieren",
  "Beim Turmbau fällt der Turm immer wieder um. Was hilft?": "etwas ändern und den neuen Versuch vergleichen",
  "Du suchst dein Buch im Zimmer. Welcher Weg ist planvoll?": "Regal für Regal der Reihe nach absuchen",
  "Zwei Spielstrategien: Eine gewinnt oft, eine selten. Welche wählst du?": "die, die oft gewinnt",
  "Was heisst «einen Lösungsweg prüfen»?": "kontrollieren, ob er wirklich zum Ziel führt",
  "Weg A braucht 12 Schritte, Weg B braucht 9 Schritte. Welcher Weg ist kürzer?": "Weg B",
  "Dein Papierflieger fliegt nicht weit. Was ist ein guter nächster Schritt?": "eine Sache ändern und neu testen",
  "«Rühre, bis der Teig glatt ist.» Was ist das?": "eine Schleife",
  "«Wenn es regnet, nimm den Schirm, sonst die Sonnenbrille.» Was ist das?": "eine Verzweigung",
  "«Hüpfe 10-mal.» Was ist das?": "eine Schleife",
  "«Wenn die Ampel grün ist, geh los.» Was ist das?": "eine Verzweigung",
  "«Sammle Karten, bis du 5 hast.» Was ist das?": "eine Schleife",
  "Woran erkennst du eine Schleife?": "etwas wird mehrmals wiederholt",
  "Woran erkennst du eine Verzweigung?": "es gibt ein Entweder-oder",
  "«Solange Musik spielt, tanze.» Was ist das?": "eine Schleife",
  "«Wenn dein Name aufgerufen wird, steh auf.» Was ist das?": "eine Verzweigung",
  "Was kann ein Computer ausführen?": "nur vordefinierte Anweisungen",
  "Was ist ein Programm?": "eine Abfolge von Anweisungen",
  "Das Programm hat einen Fehler. Was macht der Computer?": "er führt die Anweisungen trotzdem genau aus",
  "Warum macht der Computer beim zweiten Start genau das Gleiche?": "weil er nur den Anweisungen folgt",
  "Wer legt fest, was ein Programm tut?": "die Person, die es geschrieben hat",
  "Der Roboter fährt gegen die Wand. Woran liegt das am ehesten?": "an einer falschen Anweisung im Programm",
  "Kann ein Computer etwas tun, wofür es keine Anweisung gibt?": "Nein",
  "Was braucht der Computer, damit er etwas tut?": "ein Programm mit Anweisungen",
  "Du suchst ein Wort im Wörterbuch. Welcher Weg ist meistens schneller?": "in der Mitte aufschlagen und halbieren",
  "Die Karten sind nicht sortiert. Welche Suche ist möglich?": "nur die lineare Suche, eine Karte nach der anderen",
  "Was braucht die binäre Suche, damit sie funktioniert?": "sortierte Daten",
  "Wie heisst die Suche, die den Suchbereich immer halbiert?": "binäre Suche",
  "Wie heisst die Suche, die alles der Reihe nach prüft?": "lineare Suche",
  "Zwei Verfahren lösen dasselbe Problem. Worin können sie sich unterscheiden?": "in der Anzahl Schritte",
};

function solveTyped(expr) {
  let m;
  if ((m = expr.match(/^Folge der Anleitung\. Start auf Feld (\d+)\. Gehe (.+)\. Auf welchem Feld stehst du jetzt\?$/))) {
    let pos = Number(m[1]);
    for (const part of m[2].split(", ")) {
      const pm = part.match(/^(\d+) (vor|zurück)$/);
      if (!pm) return null;
      pos += pm[2] === "vor" ? Number(pm[1]) : -Number(pm[1]);
    }
    return String(pos);
  }
  if ((m = expr.match(/^Programm: Wiederhole (\d+)-mal: (?:gehe|male) (\d+) (?:Schritte|Sterne)\. Wie viele (?:Schritte|Sterne) (?:gehst|malst) du insgesamt\?$/))) {
    return String(Number(m[1]) * Number(m[2]));
  }
  if ((m = expr.match(/^Programm: x = (\d+)\. Wiederhole (\d+)-mal: x = x ([+*-]) (\d+)\. Was ist x am Ende\?$/))) {
    let x = Number(m[1]);
    for (let i = 0; i < Number(m[2]); i++) {
      if (m[3] === "+") x += Number(m[4]);
      else if (m[3] === "-") x -= Number(m[4]);
      else x *= Number(m[4]);
    }
    return String(x);
  }
  if ((m = expr.match(/^Programm: x = (\d+)\. Wenn x grösser als (\d+) ist: x = x \+ (\d+)\. Sonst: x = x - (\d+)\. Was ist x am Ende\?$/))) {
    const x = Number(m[1]);
    return String(x > Number(m[2]) ? x + Number(m[3]) : x - Number(m[4]));
  }
  if ((m = expr.match(/^Das Unterprogramm HÜPF bedeutet: (\d+) vor und (\d+) zurück\. Start auf Feld (\d+)\. Führe HÜPF (\d+)-mal aus\. Auf welchem Feld stehst du\?$/))) {
    let pos = Number(m[3]);
    for (let i = 0; i < Number(m[4]); i++) pos += Number(m[1]) - Number(m[2]);
    return String(pos);
  }
  if ((m = expr.match(/^Programm: x = (\d+)\. Das Unterprogramm PLUS bedeutet: x = x \+ (\d+)\. Rufe PLUS (\d+)-mal auf\. Was ist x am Ende\?$/))) {
    let x = Number(m[1]);
    for (let i = 0; i < Number(m[3]); i++) x += Number(m[2]);
    return String(x);
  }
  if ((m = expr.match(/^Du halbierst einen Stapel mit (\d+) Karten immer wieder, bis nur noch 1 Karte übrig ist\. Wie oft halbierst du\?$/))) {
    let n = Number(m[1]);
    let count = 0;
...
function chooseOption(expr, options) {
  let m;
  if ((m = expr.match(/^Programm: Wenn die Zahl gerade ist, sage JA, sonst sage NEIN\. Die Zahl ist (\d+)\. Was sagst du\?$/))) {
    return options.indexOf(Number(m[1]) % 2 === 0 ? "JA" : "NEIN");
  }
...
/* ── Data and copy sanity ─────────────────────────────────────────── */
{
  ...
  check("data: GA marks on a and f and h",
    STUFEN.filter((s) => s.ga).map((s) => s.id).join(",") === "a,f,h");
  const eszett = [];
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(61 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Schrittweise");
...
check("round h: GA medal for Zyklus 3", (await page.textContent(".done")).includes("Grundanspruch Zyklus 3"));
await page.click('[data-action="home"]');
await page.waitForSelector(".stufen-list");

/* ── Persistence, mistake flow, reset ─────────────────────────────── */
await page.waitForSelector(".stats-strip");
const expectedXp = roundXp("a", 8) + roundXp("f", 8) + roundXp("h", 8);
...
await page.goto(URL);
await page.waitForSelector(".stufen-list");
check("nav: breadcrumb links the overview and the app list, names the app",
  await page.locator('.crumbs a[href="../"]').count() === 1
  && await page.locator('.crumbs a[href="../ueben/"]').count() === 1
  && (await page.textContent('.crumbs [aria-current="page"]')).trim() === (await page.textContent("h1")).trim());
const navStufe = await page.getAttribute("[data-stufe]", "data-stufe");
await page.click(`[data-stufe="${navStufe}"]`);
await page.waitForSelector(".task-area");
check("nav: a round gets its own history entry and a Stufe crumb",
  (await page.evaluate(() => location.hash)) === `#stufe/${navStufe}`
  && await page.locator('.crumbs [data-nav="home"]').count() === 1
  && (await page.textContent('.crumbs [aria-current="page"]')).includes("Stufe"));
await page.goBack();
await page.waitForSelector(".stufen-list");
check("nav: browser back leaves the round for the overview", (await page.evaluate(() => location.hash)) === "");
await page.click(`[data-stufe="${navStufe}"]`);
await page.waitForSelector(".task-area");
await page.click('[data-action="abort"]');
await page.waitForSelector(".stufen-list");
const hashAfterAbort = await page.evaluate(() => location.hash);
await page.goBack();
check("nav: abort pops the round entry instead of adding one",
  hashAfterAbort === "" && page.url() === "about:blank");
await page.goto(`${URL}?stufe=${navStufe}`);
await page.waitForSelector(".task-area");
await page.goBack();
await page.waitForSelector(".stufen-list");
const addressAfterBack = await page.evaluate(() => location.search + location.hash);
await page.goBack();
check("nav: deep link keeps the overview between the round and the referrer",
  addressAfterBack === "" && page.url() === "about:blank");

/* ── Bewegung: Balken über transform, reduzierte Bewegung springt ───── */
await page.goto(URL);
await page.waitForSelector(".stufen-list");
const fillMotion = await page.evaluate(() => {
  const s = getComputedStyle(document.querySelector(".stats-strip .progress-fill"));
  return { prop: s.transitionProperty, dur: s.transitionDuration, transform: s.transform };
});
check("motion: progress bar grows via transform with a short transition",
  fillMotion.prop === "transform" && fillMotion.dur === "0.24s" && fillMotion.transform !== "none", JSON.stringify(fillMotion));
await page.emulateMedia({ reducedMotion: "reduce" });
check("motion: reduced motion drops the transition but keeps the bar",
