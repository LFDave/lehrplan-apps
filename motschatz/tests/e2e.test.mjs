// e2e.test.mjs — Playwright end-to-end tests for Motschatz.
//
// Run:
//   cd motschatz/tests && npm install && node e2e.test.mjs
//
...
// Unabhängig neu aufgeschriebenes Wörterbuch Französisch ↔ Deutsch.
const FR2DE = {
  "bonjour": "guten Tag", "merci": "danke", "oui": "ja", "non": "nein",
  "au revoir": "auf Wiedersehen", "un": "eins", "deux": "zwei", "trois": "drei",
  "rouge": "rot", "bleu": "blau", "le chat": "die Katze", "le chien": "der Hund",
  "la mère": "die Mutter", "le père": "der Vater", "l'école": "die Schule",
  "le livre": "das Buch", "le crayon": "der Bleistift", "le pain": "das Brot",
  "le lait": "die Milch", "la pomme": "der Apfel", "lundi": "Montag",
  "mardi": "Dienstag", "vert": "grün", "noir": "schwarz", "dix": "zehn",
  "il pleut": "es regnet", "le soleil": "die Sonne", "aujourd'hui": "heute",
  "demain": "morgen", "hier": "gestern", "manger": "essen", "boire": "trinken",
  "jouer": "spielen", "grand": "gross", "petit": "klein", "la semaine": "die Woche",
  "j'aime": "ich mag", "je voudrais": "ich möchte", "le week-end": "das Wochenende",
  "les devoirs": "die Hausaufgaben", "la famille": "die Familie",
  "les vacances": "die Ferien", "travailler": "arbeiten", "habiter": "wohnen",
  "acheter": "kaufen", "la ville": "die Stadt", "le matin": "der Morgen",
  "Comment ça va?": "Wie geht es dir?", "Je ne comprends pas.": "Ich verstehe nicht.",
  "Quelle heure est-il?": "Wie spät ist es?",
  "J'habite à Berne.": "Ich wohne in Bern.", "J'aime jouer au foot.": "Ich spiele gern Fussball.",
  "On se voit demain.": "Wir sehen uns morgen.",
};
const DE2FR = Object.fromEntries(Object.entries(FR2DE).map(([f, d]) => [d, f]));

function solveTyped(expr) {
  const m = expr.match(/^Übersetze auf Französisch: (.+)$/);
  return m ? DE2FR[m[1]] ?? null : null;
}

function chooseOption(expr, options) {
  let m;
  if ((m = expr.match(/^Was heisst «(.+)» auf Deutsch\?$/))) {
    return options.indexOf(FR2DE[m[1]]);
  }
  if ((m = expr.match(/^Wie heisst «(.+)» auf Französisch\?$/))) {
    return options.indexOf(DE2FR[m[1]]);
  }
  return -1;
}
...
/* ── Generator sanity against the oracle (seeded) ─────────────────── */
{
  ...
  for (const stufe of STUFEN) {
    const rng = mulberry32(43 + stufe.id.charCodeAt(0));
    for (let r = 0; r < 50; r++) {
...
check("home: title renders", (await page.textContent("h1")).trim() === "Motschatz");
check("home: all Stufen with GA badges",
  await page.locator(".stufe").count() === 4 && await page.locator(".ga-badge").count() === 2);
check("home: competency code visible", (await page.textContent('[data-stufe="b"]')).includes("FS1F.5.B.1.b"));
